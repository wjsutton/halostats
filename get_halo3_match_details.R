# Halo 3 Get match details from
# http://halo.bungie.net/Stats/GameStatsHalo3.aspx?gameid=781660958&player=THE%20TRUTH%2012
...
matches <- read.csv(paste0('data/halo3_all_matches_',gamertag_for_file,'.csv'),stringsAsFactors = F)

ids <- matches$game_id

for(i in 1:length(ids)){
  match_page <- paste0('http://halo.bungie.net/Stats/GameStatsHalo3.aspx?gameid=',ids[i],'&player=',gamertag_link)
  ...
  Sys.sleep(3)
  print(paste0("halo 3 match: ",i," downloaded."))
  
}

write.csv(match_details_df,paste0('data/halo3_match_data_',gamertag_for_file,'.csv'), row.names = F)
